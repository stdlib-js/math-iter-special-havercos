...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterHavercos, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	it = iterHavercos( uniform( 0.0, 6.28 ) );
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterHavercos( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which computes the half-value versed cosine for each iterated value', function test( t ) {
	var expected;
	var rand;
	var it;
	var N;
	...
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = havercos( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which computes the half-value versed cosine for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': havercos( values[ 0 ] ),
			'done': false
		},
		{
			'value': havercos( values[ 1 ] ),
			'done': false
		},
		{
			'value': havercos( values[ 2 ] ),
			'done': false
		},
		{
			'value': havercos( values[ 3 ] ),
			'done': true
		}
	];
